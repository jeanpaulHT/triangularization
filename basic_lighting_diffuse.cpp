...
int main() {
    ...
    vector<vec3> puntos, normales, vertex;

    auto test = Tunnel(glm::vec3(0,0,1));
    Tunnel* tunnels = &test;
    while (tunnels) {
        std::cout<<tunnels->path.size()<<'\n';
        for (auto t: tunnels->path) {
            puntos.insert(puntos.end(), t.points.begin(), t.points.end());
            normales.insert(normales.end(), t.normal.begin(), t.normal.end());
    ...
    unsigned int VBO[3], cubeVAO,EBO;
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, vertex.size() * sizeof(vec3), vertex.data(), GL_STATIC_DRAW);
